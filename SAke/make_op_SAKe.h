...
#ifndef _make_op_SAKe_h
#define _make_op_SAKe_h
 ...
 * class eoSAKeMonop MUST derive from eoMonOp<eoSAKe>
 */
#include "eoSAKeMutation.h"

/** definition of crossover (either as eoBinOp (2->1) or eoQuadOp (2->2):
 * class eoSAKeBinCrossover MUST derive from eoBinOp<eoSAKe>
 * OR
 * class eoSAKeQuadCrossover MUST derive from eoQuadOp<eoSAKe>
 */
// #include "eoSAKeBinOp.h"
// OR
#include "eoSAKeQuadCrossover.h"
...
/*
 * This function builds the operators that will be applied to the eoSAKe
 *
 ...
 * is actually templatized here: the following only applies to eoSAKe
*/

template <class EOT>
eoGenOp<EOT> & do_make_op(eoParameterLoader& _parser, eoState& _state, eoInit<EOT>& _init, int tbMin,int tbMax,
double Pme,
double Pmb,
double dHpMin,
double dHpMax)
{
  ...
  */
  ...
  /* Uncomment if necessary - and replicate as many time as you need
      mut = new eoSAKeSecondMutation<Indi>(varType  _anyVariable);
      _state.storeFunctor(mut);
...
// though you CAN add things to the checkpointing (see tutorial)
  ...
  // especially, if you have to use eoBinOp rather than eoQuad Op you�ll have
  // to modify that part
